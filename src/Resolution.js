...
function Doublon(s,t){
	...
	for(i=0;i<t.length;i++){
		if(s[0]==t[i][0] && s[1]==t[i][1]) {return(true)}
	}
	return(false);
}
...
function Solve(){
	...
	count=0;NextChute=false;
	temp=new Array();
	tableau_F=Init_Tableau();
	
	for(i=0;i<10;i++){
	for(j=0;j<7;j++){
		if(tableau_I[9-i][j]!="X" && (tableau_I[9-i][j].type=="propriete")){
		tableau_F[9-i][j]= Interact(9-i,j)}	;
		if(tableau_I[9-i][j]!="X" && (tableau_I[9-i][j].type=="figure") && (tableau_I[9-i][j].n==4) ){
...
function InteractC(i,j){
	...
	var k;	
	if(j==0 && i==9){
		if(NearCarre(i,j,-1,0)){res="Z";return(res)}
		if(NearCarre(i,j,0,1)){res="Z";return(res)}			
		} else { if(j==6 && i==9){
	               if(NearCarre(i,j,-1,0)){res="Z";return(res)}
		            if(NearCarre(i,j,0,-1)){res="Z";return(res)}		
		            } else {if(j==0){
...
{if(tableau_I[i+dx][j+dy].type=="figure" && tableau_I[i+dx][j+dy].n==4){
	return(true)
} else {return(false)}
}
...
function IsProp(i,j,dirx,diry){
	...
	//alert(dirx+"/"+diry);
	//alert(p+"/"+q);
	if((p==0||p==1) && q==0) {res.push(1);return(res)}
	if((p==2||p==7) && q==1) {res.push(2);return(res)}
	if((p==3||p==4) && q==1) {res.push(3);return(res)}
	if(p==5 && q==0) {res.push(3);return(res)}
	if(p==6 && q==0) {res.push(2);return(res)}
	if(p==3 && q==2) {res.push(4);return(res)}
	if((p==3||p==4) && q==2) {res.push(4);return(res)}
	if((p==2||p==7) && q==3) {res.push(4);return(res)}
	}
	return("X");
}
